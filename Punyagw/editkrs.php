...
$sql = "SELECT * FROM krs,matkul WHERE krs.nim='$_SESSION[username]' AND krs.smt='$smt' AND krs.tahunakd='$ta' AND matkul.kodemk=krs.kodemk ";
	...
	$hasil=$conn->query($sql);
    if($hasil->num_rows>0){
    echo "<table border=1><tr><th>Kode Mata Kuliah</th><th>Mata Kuliah</th><th>SKS</th><th>Hapus</th></tr>";
	$jmlsks = 0;
	if($hasil==TRUE){
		echo "<br>Berhasil";
    while($data=$hasil->fetch_assoc()){
		echo "<tr><td>".$data['kodemk']."</td><td>".$data['namamk']."</td><td>".$data['sks']."</td><td><a href=deletekrs.php?id=".$data['id'].">Hapus</a></td></td></tr>";
